...
public sealed class PlacementPreview : MonoBehaviour
{
    ...
    [SerializeField]
    private Color factoryWorkAreaColor =
        new Color(1f, 0.5f, 0f, 0.5f);

    private readonly List<SpriteRenderer>
        factoryBodyCells = new();

    private readonly List<SpriteRenderer>
        factoryWorkCells = new();

    // 재사용할 공장 미리보기 셀을 생성하고 초기 상태에서 숨김
    private void Awake()
    {
    ...
    // 공장 본체 9칸, 작업 영역 16칸을 미리 생성
    private void CreatePreviewObjects()
    {
        if (previewCellPrefab == null)
        {
            Debug.LogError(
                $"{name}: Preview Cell Prefab이 " +
                "연결되지 않았습니다.",
                this);
             ...
             i++)
        {
            factoryWorkCells.Add(
                CreatePreviewCell(
                    $"Factory Work Preview {i}"));
        }
    }

    // 미리보기 프리팹을 자식으로 생성하고 SpriteRenderer 반환
    private SpriteRenderer CreatePreviewCell(
        string objectName)
    {
    ...
    private static void SetListActive(
        ...
        bool isActive)
    {
        foreach (SpriteRenderer preview
                 in previews)
        {
            if (preview != null)
            {
